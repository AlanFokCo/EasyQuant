...
Usage:
    python scripts/check_doc_sync.py          # Check all docs
    python scripts/check_doc_sync.py --fix    # Auto-fix stale example refs
...
SKIP_DIRS = {"superpowers", "__pycache__", "node_modules", ".git"}
...
def check_stale_costs(verbose=False):
    """Check for outdated trading cost values in docs."""
    issues = []
    all_md = list(DOCS_DIR.rglob("*.md"))
...
def main():
    ...
    # Check 3: Stale trading costs
    print("3. Trading cost values:")
    issues = check_stale_costs(args.verbose)
    if issues:
        all_issues.extend(issues)
        for i in issues:
            print(i)
        print(f"  → {len(issues)} outdated values (run with --fix)\n")
    else:
        print("  ✓ All trading costs are current\n")

    # Check 4: Example refs exist
