...
| **胜率** | 在 HTML 中可能对应「日胜率」或「交易胜率」，含义不同 | 见下文与 [报告与指标详解](../doc/reports_and_metrics.md) |
...
**阅读建议：** 用浏览器逐个打开上述对应的 `.html` 文件（见 [`reports/README.md`](../reports/README.md)），观察相同页面结构下，不同策略的指标卡片、K 线、回撤曲线、成交表的差异。
...
各字段与 `analyze_returns` 字典键的对应关系见 [**报告与指标详解**](../doc/reports_and_metrics.md)。**仓库内已有一份示例导出索引**，便于对照真实 HTML/PNG：[`reports/README.md`](../reports/README.md)。
...
- **[Tutorial 06: RSI 均值回归策略](06_rsi_mean_reversion.md)** — 换一种策略思路，学习均值回归
- **[Example 20: 支撑阻力位组合策略](../examples/20_sr_strategy/README.md)** — 一个完整的多股票组合策略实战案例，包含预生成的回测报告，可直接查看策略表现
