...
| 路径 | 作用 |
|------|------|
| [`agent/strategy_template.py`](../agent/strategy_template.py) | 可复制的参数化策略骨架 |
| [`agent/optimizer.py`](../agent/optimizer.py) | 规则基参数搜索（**参考实现**，可独立运行） |
| [`agent/audit_log.py`](../agent/audit_log.py) | 写入 JSONL + Markdown 审计条目 |
