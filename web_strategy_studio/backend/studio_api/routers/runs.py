...
from studio_api.proc_registry import get_proc, kill as kill_proc
from studio_api.run_queue import enqueue, pending_run_ids, active_run_ids, queue_position, rate_limiter
...
async def create_run(
    ...
    source = body.source_code if body.source_code is not None else current_sv.source_code
...
async def run_stream(
    ...
    async def event_gen():
        ...
        run = await session.get(Run, run_id)
        terminal_in_db = run is not None and run.status in (
...
async def get_queue():
    ...
    return QueueStatusResponse(
        ...
        queued_runs=[QueueRunItem(run_id=rid, queue_position=i + 1) for i, rid in enumerate(pending)],
    )
