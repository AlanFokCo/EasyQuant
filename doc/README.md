...
| 做参数搜索、审计日志 | [Tutorial 10](../tutorials/10_agent_optimization.md) 与 [`agent/`](https://github.com/AlanFokCo/EasyQuant/tree/main/agent) 目录 |
...
| 2 | [报告与指标详解](reports_and_metrics.md) | HTML 报告逐屏解读、`analyze_returns` 全字段 |
| 3 | [API 速查索引](api_index.md) | 按主题跳转 `api_reference.md` 章节 |
| 4 | [常见问题 FAQ](FAQ.md) | 安装、数据、性能与排错 |
| 5 | [工具库参考](utils_reference.md) | `eqlib.utils` 技术指标与公式 |
| 6 | [PTrade/QMT 适配器](ptrade_adapter.md) | 导出到券商实盘环境 |

**教程（分步学习）：** [`tutorials/`](../tutorials/README.md)，建议从「环境与第一次运行」开始。
...
然后在 `reports/` 打开最新 `.html` 文件，确认收益曲线、回撤曲线、交易记录正常显示。

---

## 文档排版说明

本站由 **MkDocs Material** 构建。教程与手册文首的 **「本篇导览」**、`!!! tip` 等块在 **GitHub 仓库内联预览** 中可能显示为纯文本；在 **[在线文档站](https://AlanFokCo.github.io/EasyQuant/)** 中渲染最佳。
